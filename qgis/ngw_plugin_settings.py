...
class NgwPluginSettings(PluginSettings):
    ...
    def get_ngw_connection(cls, connection_name):
        ...
        return NGWConnectionSettings(
            connection_name,
            settings.value(key + '/server_url', u'', type=str),
            settings.value(key + '/username', u'', type=str),
            settings.value(key + '/password', u'', type=str)
        )
    ...
    def get_selected_ngw_connection_name(cls):
        settings = cls.get_settings()
        return settings.value('/ui/selectedConnection', u'', type=str)
